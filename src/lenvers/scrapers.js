  ...
  for(let i = 0; i < url.length; i++){
    ...
    */
    ...
    while(true){
      ...
      Array.from(
        document.querySelectorAll("a.proFeaturedImage"))
...
const lenversProductInfoesScraper = async (links, page) => {
  try {
    ...
    for(let i = 165; i < links.length; i++){
      ...
      const [titleText] = await page.$x('//*[@id="shopify-section-vela-template-product"]/div[1]/div/div[1]/div[2]/h1')
      const [materialText] = await page.$x('//*[@id="shopify-section-vela-template-product"]/div[1]/div/div[1]/div[2]/div[1]')
      /* 
      ...
      */
      if(titleText !== undefined || materialText !== undefined){
        ...
        const getTitle = await page.evaluate(text => text.innerText, titleText)
        const getMaterial = await page.evaluate(text => text.innerText, materialText)
